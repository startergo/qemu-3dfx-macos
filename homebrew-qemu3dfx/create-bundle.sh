...
# Create a bundled version of the formula with embedded patches for local testing
set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUNDLE_DIR="${SCRIPT_DIR}/formula-bundle"

echo "🔧 Creating bundled formula for local testing..."

# Clean and create bundle directory
rm -rf "$BUNDLE_DIR"
mkdir -p "$BUNDLE_DIR"/{patches,source}

# Copy patches
echo "📦 Bundling patches..."
cp "../00-qemu92x-mesa-glide.patch" "$BUNDLE_DIR/patches/"
cp "../virgil3d/0001-Virgil3D-with-SDL2-OpenGL.patch" "$BUNDLE_DIR/patches/"
cp "../virgil3d/0002-Virgil3D-macOS-GLSL-version.patch" "$BUNDLE_DIR/patches/"
cp "../virgil3d/MINGW-packages/0001-Virglrenderer-on-Windows-and-macOS.patch" "$BUNDLE_DIR/patches/"

# Copy source files
echo "📦 Bundling 3dfx sources..."
cp -r "../qemu-0/hw/3dfx" "$BUNDLE_DIR/source/"
cp -r "../qemu-1/hw/mesa" "$BUNDLE_DIR/source/"

# Copy signing script if it exists
if [ -f "../scripts/sign_commit" ]; then
    cp "../scripts/sign_commit" "$BUNDLE_DIR/"
fi

# Create a modified formula that uses the bundled files
cat > "$BUNDLE_DIR/qemu-3dfx-local.rb" << 'EOF'
class Qemu3dfxLocal < Formula
  ...
  def install
    ...
    resource("virglrenderer").stage do
      ...
      bundle_dir = "#{__dir__}"
      macos_virgl_patch = "#{bundle_dir}/patches/0001-Virglrenderer-on-Windows-and-macOS.patch"
...
echo "📝 Local test formula: $BUNDLE_DIR/qemu-3dfx-local.rb"
echo ""
echo "To test the formula:"
echo "  cd $BUNDLE_DIR"
echo "  brew install --build-from-source ./qemu-3dfx-local.rb"
